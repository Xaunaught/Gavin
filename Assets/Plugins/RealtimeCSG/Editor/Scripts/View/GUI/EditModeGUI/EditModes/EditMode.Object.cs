...
{
	internal sealed class EditModeObject : ScriptableObject, IEditMode
	{
		...
		public void SetTargets(FilteredSelection filteredSelection)
		{
			...
			for (int i = 0; i < brushes.Length; i++)
			{
				...
				var brush_cache = InternalCSGModelManager.GetBrushCache(brushes[i]);
				if (brush_cache == null ||
				...
				{
					parentModelTransforms[i] = brush_cache.childData.ModelTransform;
				}
			}
			UpdateTargetInfo();
			...
			shouldHideTool = filteredSelection.NodeTargets.Length > 0;
			if (toolIsEnabled)
		...
		void UpdateGridOrientation(Vector2 mousePosition)
		{
			...
			var intersection = movePlane.Intersection(mouseRay);
		...
		void RotateObjects(Transform[] transforms, Vector3 center, Quaternion rotationQuaternion)
		{
			...
			for (int t = 0; t < transforms.Length; t++)
			{
				transforms[t].position = rotate_matrix.MultiplyPoint(backupPositions[t]);
				transforms[t].rotation = rotationQuaternion * backupRotations[t];
			}
		...
		void UpdateRotationCircle(Matrix4x4 localToWorld, Vector2 mousePosition)
		{
			...
			rotateMousePosition	= rotatePlane.Intersection(ray);
			...
			float minSize		= 4 * handleSize * ToolConstants.minRotateRadius;
		...
		void RenderHighlightedEdge(int highlightEdge)
		{
			...
			PaintUtility.DrawLine(activeSpaceMatrices.activeLocalToWorld, localPoint1, localPoint2, ToolConstants.oldThickLineScale, ColorSettings.BoundsEdgeHover);
			...
			try
			{
				...
				if (Event.current.type == EventType.Repaint)
				{
					...
					if (lastLineMeshGeneration != InternalCSGModelManager.MeshGeneration)
					{
						...
						CSGRenderer.DrawSelectedBrushes(zTestLineMeshManager, noZTestLineMeshManager, brush_ids, brush_translations, ColorSettings.SelectedOutlines, ToolConstants.lineScale);
					}
					...
					if (!SceneDragToolManager.IsDraggingObjectInScene)
					{
						...
						{
							PaintUtility.DrawLines(activeSpaceMatrices.activeLocalToWorld, targetLocalPoints, BoundsUtilities.AABBLineIndices, ToolConstants.oldLineScale * 2.0f, meshEdgeOutlineColor);
							...
							PaintUtility.DrawLines(activeSpaceMatrices.activeLocalToWorld, targetLocalPoints, BoundsUtilities.AABBLineIndices, ToolConstants.oldLineScale, boundOutlinesColor);
						}
				...
				switch (Event.current.type)
				{
					...
					case EventType.KeyUp:
					{
						...
						if (Keys.RotateSelectionLeft.IsKeyPressed())
						{
							...
							RotateObjects(topTransforms, center, Vector3.up,  RealtimeCSG.CSGSettings.SnapRotation);
							Event.current.Use();
							break;
						}
						...
						if (Keys.MoveSelectionLeft   .IsKeyPressed()) { UpdateTargetInfo(); MoveObjects(topTransforms, Vector3.right * RealtimeCSG.CSGSettings.SnapVector.x); Event.current.Use(); break; }
						if (Keys.MoveSelectionRight  .IsKeyPressed()) { UpdateTargetInfo(); MoveObjects(topTransforms, Vector3.left * RealtimeCSG.CSGSettings.SnapVector.x); Event.current.Use(); break; }
						if (Keys.MoveSelectionBack   .IsKeyPressed()) { UpdateTargetInfo(); MoveObjects(topTransforms, Vector3.forward * RealtimeCSG.CSGSettings.SnapVector.z); Event.current.Use(); break; }
						if (Keys.MoveSelectionForward.IsKeyPressed()) { UpdateTargetInfo(); MoveObjects(topTransforms, Vector3.back * RealtimeCSG.CSGSettings.SnapVector.z); Event.current.Use(); break; }
						if (Keys.MoveSelectionDown   .IsKeyPressed()) { UpdateTargetInfo(); MoveObjects(topTransforms, Vector3.down * RealtimeCSG.CSGSettings.SnapVector.y); Event.current.Use(); break; }
						if (Keys.MoveSelectionUp     .IsKeyPressed()) { UpdateTargetInfo(); MoveObjects(topTransforms, Vector3.up * RealtimeCSG.CSGSettings.SnapVector.y); Event.current.Use(); break; }
								...
								GUIUtility.hotControl == 0)
							{
								...
								if (activeTransform != null)
								{
									...
									for (int t = 0; t < brushes.Length; t++)
									{
										...
										var modelTranslation = model_transform.position;

										BrushIntersection intersection;
										if (SceneQueryUtility.FindBrushIntersection(brush, modelTranslation, ray_start, ray_end, out intersection))
										{
									...
									//if (!Camera.current.orthographic)
									{
											...
											!cloneDragKeyPressed)
										{
											...
											{
												float radius = renderLocalCenterPointSizes[t] * ToolConstants.hoverHandleScale;
												if (radius == 0)
													continue;
														...
														} else
														{
															for (int i = 0; i < 2; i++)
															{
																...
																if (renderLocalCenterPointSizes[side] == 0)
																	continue;
																float radius = renderLocalCenterPointSizes[side] * ToolConstants.hoverHandleScale;
																if (radius != 0)
																{
				...
				for (int i = 0; i < targetBoundCenterIDs.Length; i++)
				{
					...
					switch (type)
					{
						case EventType.MouseDrag:
						{
							...
							var worldIntersection = movePlane.Intersection(mouseRay);
						...
						case EventType.MouseDrag:
						{
							...
							if (firstMove)
							{
								...
								for (var b = 0; b < topTransforms.Length; b++)
								{
									...
									if (gameObject.activeInHierarchy &&
										!SelectionUtility.IsPrefab(gameObject))
									{
							...
							if (useRaySnapping)
							{
								...
								finally
								{
									...
									{
										if (brushes[b])
											BrushOutlineManager.ForceUpdateOutlines(brushes[b].brushID);
									}
					...
					for (int i = 0; i < targetBoundEdgeIDs.Length; i++)
					{
						...
						switch (type)
						{
							case EventType.MouseDrag:
							{
								...
								firstMove = false;
								var rotatePlane = new CSGPlane(rotateNormal, boundsCenter);
								rotateMousePosition = rotatePlane.Intersection(mouseRay);
		...
		public void OnInspectorGUI(EditorWindow window, float height)
		{
			...
			{
				EditmodeObjectGUI.OnInspectorGUI(window, height);
			}
		...
		{
			return EditmodeObjectGUI.GetLastSceneGUIRect(this);
		}
		...
		public bool OnSceneGUI(Rect windowRect)
		{
			...
			EditmodeObjectGUI.OnSceneGUI(windowRect, this);
			return false;
		}
	}
}