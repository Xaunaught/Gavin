...
//		[NonSerialized] bool			onHandle		= false;
		...
		public void Commit()
		{
			...
			try
			{
				...
				for (var t = 0; t < clipBrushes.Length; t++)
				{
					...
					BrushOutlineManager.ForceUpdateOutlines(clipBrushes[t].brushID);
				}
		...
		int RememberTargetData(CSGBrush[] addedBrushes = null)
		{
			...
			for (int srcIndex = srcBrushes.Length - 1; srcIndex >= 0; srcIndex--)
			{
				...
				backupData[dstIndex].wireFrame		= BrushOutlineManager.GetBrushOutline(srcBrush.brushID);
				
				var brush_cache		= InternalCSGModelManager.GetBrushCache(srcBrush);
				if (brush_cache == null ||
					brush_cache.childData == null ||
					!brush_cache.childData.Model)
				{
					backupData[dstIndex].position	= MathConstants.zeroVector3;
					backupData[dstIndex].model		= null;
					continue;
				}
				
				backupData[dstIndex].position	= brush_cache.compareTransformation.modelLocalPosition + brush_cache.childData.ModelTransform.position;
				backupData[dstIndex].model		= brush_cache.childData.Model;
			}
			return offset;
		}
...
//				prevClipPlaneValid = clipPlaneValid;
		...
		void UpdateMouseIntersection()
		{
			...
			} else
			{
				...
				if (movePlane.TryIntersection(world_ray, out intersectionPoint))
...
//		MouseCursor lastUsedCursor = MouseCursor.Arrow;
		...
		void CreateFirstPoint()
		{
			...
			{
				var brush_cache = InternalCSGModelManager.GetBrushCache(currentOnBrush);
				if (brush_cache != null &&
					brush_cache.childData != null &&
					brush_cache.childData.ModelTransform != null)
				{
					...
					BrushIntersection intersectionFar;
					if (SceneQueryUtility.FindBrushIntersection(currentOnBrush, brush_cache.childData.ModelTransform.position, ray_start, ray_end, out intersectionFar))
					{
		...
		{					
			...
			if (forceWireframeUpdate)
			{
				...
				if (clipPlaneValid && backupData.Length > 0)
				{
					...
					var translations	= new List<Vector3>(backupData.Length);
					for (int i = 0; i < backupData.Length; i++)
					{
						...
						var brush_translation = backupData[i].position;
						wireframes.Add(backupData[i].wireFrame);
						translations.Add(brush_translation);
					}
				...
				if (clipBrushes != null)
				{
					...
					var translations = new List<Vector3>(clipBrushes.Length);
					for (int i = 0; i < clipBrushes.Length; i++)
					{
						...
						ids.Add(brush.brushID);
						translations.Add(brush_translation);
						//CSGRenderer.DrawSelectedBrush(brush.brushID, brush_translation, ColorSettings.SelectedOutlines, ToolConstants.thickLineScale);
					}
					if (ids.Count > 0)
					{
						CSGRenderer.DrawSelectedBrushes(zTestLineMeshManager1, noZTestLineMeshManager1,
							ids.ToArray(), translations.ToArray(),
							ColorSettings.SelectedOutlines, ToolConstants.thickLineScale);
					}
				}
			}
...
//			onHandle = false;
					...
					{ 
						...
						//if (TryFindIntersection(world_ray, out intersectionPoint))
						{
							...
							if (this.currentOnBrush == null)
								PaintUtility.DiamondDotCap(-1, intersectionPoint, MathConstants.identityQuaternion, handleSize * 1.2f * ToolConstants.handleScale);
							else
								PaintUtility.SquareDotCap(-1, intersectionPoint, MathConstants.identityQuaternion, handleSize * ToolConstants.handleScale);
							Handles.matrix = prevMatrix;
						}// else visualSnappedLines = null;
					}

					if (visualSnappedLines != null)
						PaintUtility.DrawLines(visualSnappedLines.ToArray(), ToolConstants.oldThinLineScale * 2.0f, ColorSettings.SnappedEdges);
					break;
				}
				...
				case EventType.MouseDrag:
				{
					if (GUIUtility.hotControl == planeCreationID)
					{
						...
						if (editMode == EditMode.CreatingPoint1)
						{
							...
							if (magnitude > handleSize * (ToolConstants.handleScale / 10.0f))
							{
					...
					{ 
						...
						{ 
							...
							for (int i = 0; i < pointsShown; i++)
							{
								...
								PaintUtility.SquareDotCap(-1, points[i], MathConstants.identityQuaternion, handleSize * ToolConstants.handleScale);
							}
							Handles.matrix = prevMatrix;
						}
		...
		public void HandleEditPointEvents(Rect sceneRect)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				sizes[i] = CSG_HandleUtility.GetHandleSize(points[i]) * ToolConstants.handleScale;
