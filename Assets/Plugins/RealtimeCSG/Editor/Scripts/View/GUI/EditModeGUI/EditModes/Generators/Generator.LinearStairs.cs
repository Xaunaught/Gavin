...
{
	[Serializable]
    internal sealed class GeneratorLinearStairs : GeneratorBase, IGenerator
	{
		...
		[SerializeField] GeneratorLinearStairsSettings settings = new GeneratorLinearStairsSettings();
		...
		public float StepDepth
		{
			...
			set
			{
				...
				Undo.RecordObject(this, "Modified Linear Stairs Step Depth");
		...
		public float StepHeight
		{
			...
			set
			{
				if (settings.StepHeight == value)
					return;
				Undo.RecordObject(this, "Modified Linear Stairs Step Height");
		...
		public int TotalSteps
		{
			...
			set
			{
				if (settings.TotalSteps == value)
					return;
				Undo.RecordObject(this, "Modified Linear Stairs Total Steps");
		...
		public float	StairsWidth
		{
			...
			set
			{
				...
				Undo.RecordObject(this, "Modified Linear Stairs Width");
		...
		public float	StairsDepth
		{
			...
			set
			{
				...
				Undo.RecordObject(this, "Modified Linear Stairs Depth");
		...
		public float	StairsHeight
		{
			...
			set
			{
				...
				Undo.RecordObject(this, "Modified Linear Stairs Height");
		...
		public float ExtraDepth
		{
			...
			set
			{
				...
				Undo.RecordObject(this, "Modified Linear Stairs Extra Depth");
		...
		public float ExtraHeight
		{
			...
			set
			{
				...
				Undo.RecordObject(this, "Modified Linear Stairs Extra Height");
		...
		{
			get { return settings.StairsBottom; }
			set
			{
				if (settings.StairsBottom == value)
					return;
				
				Undo.RecordObject(this, "Modified Stairs Bottom");
				settings.StairsBottom = value;
		...
		{
			ResetVisuals();
			RealtimeCSG.CSGGrid.SetForcedGrid(new CSGPlane(fromGridQuaternion * Vector3.up, settings.bounds.Min));
			switch (editMode)
			{
				default:
				{
		...
		public Quaternion GetPlaneRotation()
		{
			...
			if (xznormal.y < 0.5f)
			{
				xznormal.y = 0;
				xznormal.Normalize();
				return Quaternion.LookRotation(xznormal, Vector3.up);
			}
			
			return fromGridQuaternion;
		}
		...
		public Quaternion GetWorldToLocalRotation()
		{
			...
			if (xznormal.y < 0.5f)
			{
				xznormal.y = 0;
				xznormal.Normalize();
				return Quaternion.Inverse(Quaternion.LookRotation(xznormal, Vector3.up));
			}
			
			return toGridQuaternion;
		}
		...
		public bool OnShowGUI(bool isSceneGUI) { return GeneratorLinearStairsGUI.OnShowGUI(this, isSceneGUI);	}
		...
		void PaintBounds()
		{
			...
			PaintUtility.DrawLines(localToWorld, volume, BoundsUtilities.AABBLineIndices, ToolConstants.oldLineScale, boundOutlinesColor);
		...
		protected override void HandleEditShapeEvents(Rect sceneRect)
		{
			...
			EditorGUI.BeginChangeCheck();
			{
				...
				widthDirection	= /*worldToLocalRotation * fromGridQuaternion */ Vector3.right;
				heightDirection	= /*worldToLocalRotation * fromGridQuaternion */ Vector3.up;
				lengthDirection	= /*worldToLocalRotation * fromGridQuaternion */ Vector3.forward;

				//Debug.Log(widthDirection + " " + heightDirection + " " + lengthDirection);
				...
				ignoreModelRotation = true;
				brushRotation = GetPlaneRotation();
				brushPosition = localToWorldRotation * 
									settings.bounds.Min;
				...
				PaintUtility.DrawLine(localToWorld, topFrom, topTo, ToolConstants.oldThickLineScale, topColor);
				...
				PaintUtility.DrawLine(localToWorld, bottomFrom, bottomTo, ToolConstants.oldThickLineScale, bottomColor);
				
				
				PaintUtility.DrawLine(localToWorld, topFrom, bottomFrom, ToolConstants.oldThickLineScale, ColorSettings.MeshEdgeOutline);
				PaintUtility.DrawLine(localToWorld, topTo  , bottomTo,   ToolConstants.oldThickLineScale, ColorSettings.MeshEdgeOutline);
				...
				{ 
					var topDirection	= localToWorld.MultiplyVector( Vector3.forward );
				...
				{ 		
					var bottomDirection	= localToWorld.MultiplyVector( Vector3.up );
		...
		void UpdateSizes()
		{
			...
			ignoreModelRotation = true;
			brushRotation = GetPlaneRotation();// * fromGridQuaternion;
