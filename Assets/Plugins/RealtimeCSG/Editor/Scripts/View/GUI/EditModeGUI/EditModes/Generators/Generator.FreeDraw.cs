	...
	{	
		...
		{			
			...
			if (Event.current != null &&
				Event.current.type == EventType.Repaint)
			{
				for (int i = 0; i < curvedVertices.Length; i++)
				{
					float handleSize = CSG_HandleUtility.GetHandleSize(curvedVertices[i]) * 0.5f;
					PaintUtility.SquareDotCap(-1, curvedVertices[i], Quaternion.identity, handleSize);
				}
			}
