...
namespace RealtimeCSG
{
	internal sealed class EditModeSurface : ScriptableObject, IEditMode
	{
		...
		[NonSerialized] int             hoverOnBrushIndex           = -1;
		...
		EditMode SetHoverOn(EditMode editModeType, int target, int index = -1)
		{
			...
			hoverOnBrushIndex = brushTargets[target].brushID;
		...
		void DrawOutlines()
		{
			...
			if (forceOutlineUpdate ||				
				...
				prevHoverBrushIndex   != hoverOnBrushIndex)
			{
				prevHoverSurfaceIndex = hoverOnSurfaceIndex;
				prevHoverBrushIndex = hoverOnBrushIndex;
			...
			try
			{
				...
				switch (Event.current.type)
				{
					...
					case EventType.MouseMove:
					{
						...
						{
							//if (hoverOnSurfaceIndex != intersection.surfaceIndex || hoverOnTarget != intersection.brushID)
							{
								SceneView.RepaintAll();
							}
						}
					...
					case EventType.Layout: 
					{
						...
						if (hoverControl == -1)
						{
							...
							if (!mouseIsDragging &&
								...
								SceneQueryUtility.FindWorldIntersection(ray_start, ray_end, out intersection, ignoreInvisibleSurfaces: true, ignoreUnrenderables: !wireframeShown))
							{
					...
					for (int s = 0; s < surfaceControls.Length; s++)
					{
						...
						switch (eventType)
						{
							case EventType.MouseDrag:
							{
								...
								{
									Debug.Log("Please change the Unity Tool to Move or Rotate");
								}
								break;
							}
							case EventType.MouseUp:
