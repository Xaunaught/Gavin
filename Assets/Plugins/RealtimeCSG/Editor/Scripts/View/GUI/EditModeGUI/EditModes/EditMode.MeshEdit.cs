	...
	internal sealed class EditModeMeshEdit : ScriptableObject, IEditMode
	{
		...
		private void SelectMarquee(SceneView sceneView, Rect rect, SelectionType selectionType)
        {
            ...
            try
            {
				...
				var selectedPoints  = SceneQueryUtility.GetPointsInFrustum(sceneView, frustum.Planes, _brushSelection.Brushes, _brushSelection.States, cameraOrtho);
		...
		private EditMode HoverOnPolygon(ControlMeshState meshState, int brushIndex, int polygonIndex)
		{
			...
			//Debug.Log(tangent + " " + binormal + " | " + currPlane.Tangent + " " +  currPlane.BiNormal);
			*/
		...
		{
			EditModeManager.EditMode = ToolEditMode.Mesh;
...
//			var brushLocalToWorld = targetMeshState.BrushTransform.localToWorldMatrix;
			...
			EditModeManager.GenerateFromSurface(brush, polygonPlane, worldNormal, points, pointIndices, smoothingGroups, drag);
		}
				...
				{ 
					...
					if (DrawBrushPoints)
					{
						for (var t = 0; t < _brushSelection.Brushes.Length; t++)
						{
							...
							//Debug.Log("D:" + meshState.WorldPointSizes[0]);
					...
					if (DrawPolygonPoints)
					{
						for (var t = 0; t < _brushSelection.Brushes.Length; t++)
						{
							...
							//Debug.Log("D:" + meshState.PolygonCenterPointSizes[0]);
		...
		void UpdateChamferIntersectionPoint()
		{
			...
			{
				var tryIntersection = chamferPlanes[p0].Intersection(mouseRay);
				bool tryFound = true;
		...
		void HandleChamfering()
		{
			...
			switch (type)
			{
				...
				case EventType.Repaint:
				{
					...
					PaintUtility.SquareDotCap(-1, chamferIntersectionPoint, MathConstants.identityQuaternion, CSG_HandleUtility.GetHandleSize(chamferIntersectionPoint) * ToolConstants.handleScale);
					break; 
				}
			}
		}
