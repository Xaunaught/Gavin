...
namespace RealtimeCSG
{
	...
	{
#if EVALUATION
		const string NativePluginName = "RealtimeCSG-Evaluation-Native";
#else
		...
		private static extern bool RayCastIntoModel(Int32 modelID,
													 ...
													 [In] IntPtr		ignoreNodeIndices,
													 Int32				ignoreNodeIndexCount,
													 out float			distance,
													 out Int32			uniqueID,
													 out Int32			brushID,
													 out Int32			surfaceIndex,
													 out Int32			NativeTexGenIndex,
											 ...
											 CSGBrush[]				ignoreBrushes = null)
		{
			...
			bool result = RayCastIntoModel(model.modelID,
										   ref rayStart,
										   ref rayEnd,
			...
			intersection = new BrushIntersection
			{
				distance			= distance,
				brushID				= brushID,
												  ...
												  CSGBrush[]				ignoreBrushes = null)
        {
			...
			Int32[]		modelID				= new Int32[intersectionCount];
			...
			GCHandle modelIDHandle				= GCHandle.Alloc(modelID, GCHandleType.Pinned);
			...
			IntPtr modelIDPtr				= modelIDHandle.AddrOfPinnedObject();
			...
			var intersectionList = new List<BrushIntersection>();			
			for (int i = 0, t = 0; i < intersectionCount; i++, t+=3)
			{
				...
				intersectionList.Add(new BrushIntersection
				{
					distance			= distance[i],
					brushID				= brushID[i],
		...
		private static bool RayCastMulti(CSGModel[]					models,
									     Vector3					rayStart,
										 Vector3					rayEnd,
										 bool						ignoreInvisiblePolygons,
										 out BrushIntersection[]	intersections,
										 CSGBrush[]					ignoreBrushes = null)
        {
			...
			int prevIntersectionCount = 1024;

			Single[]	distance			= new Single[prevIntersectionCount];
			Int32[]		uniqueID			= new Int32[prevIntersectionCount];
			Int32[]		brushID				= new Int32[prevIntersectionCount];
			Int32[]		modelID				= new Int32[prevIntersectionCount];
			Int32[]		surfaceIndex		= new Int32[prevIntersectionCount];
			Int32[]		texGenIndex			= new Int32[prevIntersectionCount];
			Vector2[]	surfaceIntersection = new Vector2[prevIntersectionCount];
			Vector3[]	worldIntersection	= new Vector3[prevIntersectionCount];
			byte[]		surfaceInverted		= new byte[prevIntersectionCount];
			CSGPlane[]	planes				= new CSGPlane[prevIntersectionCount];
			...
			for (int m = 0; m < models.Length; m++)
			{
				...
				var modelRayStart		= rayStart - modelTranslation;
				var modelRayEnd			= rayEnd   - modelTranslation;
				Int32 intersectionCount = RayCastIntoModelMultiCount(model.modelID, 
																	 ref modelRayStart,
																	 ref modelRayEnd,
																	 ignoreInvisiblePolygons,
				...
				if (intersectionCount > prevIntersectionCount)
				{
					...
					distance					= new Single[intersectionCount];
					uniqueID					= new Int32[intersectionCount];
					brushID						= new Int32[intersectionCount];
					modelID						= new Int32[intersectionCount];
					surfaceIndex				= new Int32[intersectionCount];
					texGenIndex					= new Int32[intersectionCount];
					surfaceIntersection			= new Vector2[intersectionCount];
					worldIntersection			= new Vector3[intersectionCount];
					surfaceInverted				= new byte[intersectionCount];
					planes						= new CSGPlane[intersectionCount];
					
					distanceHandle				= GCHandle.Alloc(distance, GCHandleType.Pinned);
					uniqueIDHandle				= GCHandle.Alloc(uniqueID, GCHandleType.Pinned);
					brushIDHandle				= GCHandle.Alloc(brushID, GCHandleType.Pinned);
					modelIDHandle				= GCHandle.Alloc(modelID, GCHandleType.Pinned);
					surfaceIndexHandle			= GCHandle.Alloc(surfaceIndex, GCHandleType.Pinned);
					texGenIndexHandle			= GCHandle.Alloc(texGenIndex, GCHandleType.Pinned);
					surfaceIntersectionHandle	= GCHandle.Alloc(surfaceIntersection, GCHandleType.Pinned);
					worldIntersectionHandle		= GCHandle.Alloc(worldIntersection, GCHandleType.Pinned);
					surfaceInvertedHandle		= GCHandle.Alloc(surfaceInverted, GCHandleType.Pinned);
				...
				IntPtr surfaceInvertedPtr		= surfaceInvertedHandle.AddrOfPinnedObject();
				IntPtr planesPtr				= planesHandle.AddrOfPinnedObject();
				var result = RayCastMultiGet(intersectionCount, distancePtr, uniqueIDPtr, brushIDPtr, modelIDPtr, surfaceIndexPtr, texGenIndexPtr, surfaceIntersectionPtr, worldIntersectionPtr, surfaceInvertedPtr, planesPtr);
				...
				for (int i = 0, t = 0; i < intersectionCount; i++, t+=3)
				{
					...
					intersectionList.Add(new BrushIntersection
					{
						...
						texGenIndex			= texGenIndex[i],
						surfaceIntersection = surfaceIntersection[i],
						worldIntersection	= worldIntersection[i] + modelTranslation,
						surfaceInverted		= (surfaceInverted[i] == 1),
						plane				= planes[i].Translated(modelTranslation),
						model				= model
					});
				}
			}
		...
		private static bool RayCastIntoBrushSurface(Int32					brushID, 
												    ...
												    Vector3					rayStart,
												    Vector3					rayEnd,
												    out SurfaceIntersection	intersection)
        {
			if (brushID == -1)
			{
				intersection = null;
			...
			var result = new SurfaceIntersection
			{
				texGenIndex			= texGenIndex,
				surfaceIntersection = surfaceIntersection,
				worldIntersection	= worldIntersection,
									 ...
									 TexGenFlags[]		texgenFlags)
		{
			...
			return SetBrush(brushID, modelID, parentID, operation, ref localToWorld,
		...
		[DllImport(NativePluginName, CallingConvention = CallingConvention.Cdecl)]
		private static extern bool SetBrushLocalToWorld(Int32 brushID,
														[In] ref Matrix4x4 localToWorldSpace);
		...
		[DllImport(NativePluginName, CallingConvention=CallingConvention.Cdecl)]
		private static extern bool ApplyAnyChanges();

		[DllImport(NativePluginName, CallingConvention = CallingConvention.Cdecl)]
		private static extern bool ModelMeshesNeedUpdate();
        ...
        public static void RegisterExternalMethods()
		{
			...
			methods.SetBrushLocalToWorld		= SetBrushLocalToWorld;
			...
			methods.ApplyAnyChanges				= ApplyAnyChanges;
			methods.ModelMeshesNeedUpdate		= ModelMeshesNeedUpdate;
