...
namespace RealtimeCSG
{
	internal static class InternalCSGModelManager
	{
		...
		public static void InitOnNewScene()
		{
			...
			if (External == null) NativeMethodBindings.RegisterExternalMethods();
		...
		static void FindParentOperationAndModel(Transform opTransform, out CSGOperation parentOp, out CSGModel parentModel)
		{
			...
			while (iterator)
			{
				...
				if (currentNodeObj)
				{
					...
					parentOp = currentNodeObj as CSGOperation;
		...
		static void RegisterBrush(CSGBrush brush)
		{
			...
			{
				var localToWorld	= brushCache.compareTransformation.localToModelMatrix;
		...
		static void RegisterOperation(CSGOperation op)
		{
			...
			External.SetOperationHierarchy(opID, modelID, parentID);
		...
		private static void RegisterModel(CSGModel model)
		{
			...
			External.SetModelEnabled(model.modelID, modelCache.IsEnabled);
		...
		public static void CheckTransformChanged(bool checkAllBrushes = false)
		{
			...
			for (int i = 0; i < Operations.Length; i++)
			{
				...
				var operationCache = _operationCaches[operation.operationID];
			...
			for (int i = 0; i < Models.Length; i++)
			{
				...
				model.cachedPosition = model.cachedTransform.position;
			}
			...
			for (int brushID = 0; brushID < _brushCaches.Length; brushID++)
			{
					...
					prevTransformMatrix.m23 != currentLocalToWorldMatrix.m23)
				{
					...
					brushCache.compareTransformation.localToModelMatrix = modelTransform.worldToLocalMatrix * 
						brushCache.compareTransformation.localToWorldMatrix;
						
					var modelLocalPosition	= brushTransform.position;
					...
					var localToWorldMatrix = brushCache.compareTransformation.localToModelMatrix;
					External.SetBrushLocalToWorld(brushID, ref localToWorldMatrix);
		...
		{
			if (External != null &&
				External.ApplyAnyChanges != null)
				External.ApplyAnyChanges();
		}
		#endregion
		...
		public static bool UpdateMeshes(System.Text.StringBuilder text = null, bool forceUpdate = false)
		{
			...
			try
			{
				...
				var needToUpdateMesh = External.ModelMeshesNeedUpdate();
				//Profiler.EndSample();
